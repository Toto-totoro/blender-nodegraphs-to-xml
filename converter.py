...
def convert_node_graph_to_xml(node_graph, root, graph_id) -> int:
    ...
    nodegroup_element = ET.SubElement(
        ...
        id=str(graph_id),
        type="Material" if is_material else "NodeGroup",
    )
    ...
    for node in node_graph.nodes if not is_material else node_graph.node_tree.nodes:
        ...
        node_is_nodegroup = (
            node.bl_idname == "ShaderNodeGroup" or node.bl_idname == "GeometryNodeGroup"
        )
        ...
        node_element = ET.SubElement(
            nodegroup_element, "Node", name=node.name, type=node.bl_idname
        )
    ...
    for link in node_graph.links if not is_material else node_graph.node_tree.links:
        ...
        if link.from_socket is None or link.to_socket is None:
            print(
                f"Link from {link.from_node.name} to {link.to_node.name} has invalid sockets. Skipping."
            )
            continue
...
def convert_nodegroup_node_to_xml(node, parent_element, graph_id):
    ...
    inner_output_node_elements = parent_element.xpath(
        f"Graph[@id='{graph_id}']/Node[@type='NodeGroupOutput']"
    )
    ...
    wrapperIN_node_element = ET.SubElement(
        parent_element,
        "Node",
        name=node.name + "_WrapperIn",
        type=node.bl_idname + "Input",
    )
    wrapperOUT_node_element = ET.SubElement(
        parent_element,
        "Node",
        name=node.name + "_WrapperOut",
        type=node.bl_idname + "Output",
    )

    inner_input_nodes = [
        n for n in node.node_tree.nodes if n.bl_idname == "NodeGroupInput"
    ]
    inner_output_nodes = [
...
def convert_node_properties_to_xml(node, node_element, filter_unnecessary=None):
    ...
    # necessary, since ShaderNodeValue (legacy node) the only (known) node that provides its value in just the output
    ...
    for prop_name in node.bl_rna.properties.keys():
        ...
        elif isinstance(prop, (bpy.types.GeometryNodeTree, bpy.types.ShaderNodeTree)):
...
def connect_wrapperIN_to_innerIN(
    ...
    for socket_index, output_socket in enumerate(reference_node.outputs):
        ...
        for inner_node, inner_input_node_element, inner_property_map in zip(
            inner_nodes, inner_input_node_elements, inner_property_maps
        ):
            target_socket = inner_node.outputs[
                socket_index
            ...
            create_connection_element(
                wrapper_node_element.getparent(), outer_id, inner_id
            )
